...
export default function NewBookingLinkPage() {
    ...
    const fetchMembers = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // 現在のユーザーを取得（最初のメンバーと仮定、または別のAPIで取得）
  ...
  const handleAddField = useCallback(() => {
    ...
    if (!newField.label) {
      console.log('newField.label is empty, returning')
      return
    }
    
    setFormFields(prev => {
      console.log('prev formFields:', prev)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // formFieldsの現在の値を確実に取得
    const currentFormFields = formFields
    console.log('Creating booking link with formFields:', currentFormFields)
    console.log('formFields count:', currentFormFields.length)
    console.log('Creating booking link with requiredMembers:', requiredMembers)
    console.log('requiredMembers count:', requiredMembers.length)
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="bg-white border-b border-gray-200 pt-6 pb-8 px-8">
        ...
        <p className="text-gray-500 text-sm">商談・面談・相談など、用途に合わせてリンクを発行</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-gray-700 mb-2">
                  予約リンクURL <span className="text-red-500">*</span>
                </label>
